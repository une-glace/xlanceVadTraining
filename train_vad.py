...
def train():
    ...
    # 3. Data
    # Paths to the .scp files generated by run_preprocessing.py
    speech_scp = os.path.join("WenetSpeech_16k", "wav.scp")
    noise_scp = os.path.join("musan_16k", "wav.scp")
    
    if not os.path.exists(speech_scp) or not os.path.exists(noise_scp):
        if is_master:
            print(f"Error: {speech_scp} or {noise_scp} not found.")
            print("Please run 'python run_preprocessing.py' first.")
            wandb.finish()
        cleanup_distributed()
        return
